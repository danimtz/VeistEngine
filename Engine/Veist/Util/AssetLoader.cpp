...
#include "Veist/Scenes/ECS/Components/MeshComponent.h"
...
namespace Veist
{
	...
	
	////													New GLTF asset loading																				////
	


	static void parseGLTFMaterial(tinygltf::Material& gltf_material, tinygltf::Model& model, std::vector<std::shared_ptr<ImageBase>>& material_textures, std::string& folder_directory)
	{
		...
		if(albedo_idx != -1)
		{
			...
			int tex_src = model.textures[albedo_idx].source;
			std::string uri = model.images[tex_src].uri;
			std::string folder = folder_directory;
			std::shared_ptr<Texture> albedo = AssetLoader::loadTextureFromFile(folder.append(uri).c_str(), { VK_FORMAT_R8G8B8A8_SRGB });
			CONSOLE_LOG(" ===================== ALBEDO TEXTURE");
		...
		if (normal_idx != -1)
		{
			int tex_src = model.textures[normal_idx].source;
			std::string uri = model.images[tex_src].uri;
			std::string folder = folder_directory;
			std::shared_ptr<Texture> normal = AssetLoader::loadTextureFromFile(folder.append(uri).c_str(), { VK_FORMAT_R8G8B8A8_UNORM });
			CONSOLE_LOG(" ===================== NORMAL TEXTURE");
		...
		//Occlusion/Metallic/roughness (ASSUMES OCCLUSION IN IN METALLIC ROUGHNESS TEXTURE. as in gltf2.0 spec)
		int roughness_metallic_idx = gltf_material.pbrMetallicRoughness.metallicRoughnessTexture.index;
		if (roughness_metallic_idx != -1)
		{
			int tex_src = model.textures[roughness_metallic_idx].source;
			std::string uri = model.images[tex_src].uri;
			std::string folder = folder_directory;
			std::shared_ptr<Texture> occlusionRoughnessMetallic = AssetLoader::loadTextureFromFile(folder.append(uri).c_str(), { VK_FORMAT_R8G8B8A8_UNORM });
			CONSOLE_LOG(" ===================== OCC TEXTURE");
		...
		//Emmissive TODO enable emmisive

		int emmissive_idx = gltf_material.emissiveTexture.index;
		if (false/*emmissive_idx != -1*/)
		{
			int tex_src = model.textures[emmissive_idx].source;
			std::string uri = model.images[tex_src].uri;
			std::string folder = folder_directory;
			std::shared_ptr<Texture> emmissive = AssetLoader::loadTextureFromFile(folder.append(uri).c_str(), { VK_FORMAT_R8G8B8A8_SRGB });
			CONSOLE_LOG(" ===================== EMMISSIVE TEXTURE");
			
			material_textures.emplace_back(emmissive);
		}

		//TODO add more settings based on gltf_material such as alpha value, double sided, emmissive value etc
	...
	static void processGLTFMeshPrimitive(tinygltf::Primitive& primitive, tinygltf::Model& model, std::vector<SubMesh>& sub_meshes, std::string& folder_directory)
	{

		//Parse vertices/data

		//Assert primitive mode is triangles (4)
		if (primitive.mode != 4)
		{
			CRITICAL_ERROR_LOG("Unsupported gltf primitive mode");
		}

		//Step 1: Iterate through attributes in mesh primitive
		struct AttributeInfo
		{
			AttributeInfo(tinygltf::Accessor accessor_in, VertexAttribute attribute) : accessor(accessor_in), vattribute(attribute) {};
			tinygltf::Accessor accessor;
		...
		for (uint32_t i = 0; i < attribute_count; i++)
		{
			...
			for (auto it = vattrib_map.begin(); it != vattrib_map.end(); it++)
			{
				...
				uint32_t buffer_offset =  buffer_view_byte_offset + accessor_byte_offset;
		...
		if (idx_buff_accessor.componentType != 5123)
		{
			CRITICAL_ERROR_LOG("Index buffer for GLTF file is not 16 bit");
		}

		//slice index buffer data
		uint32_t idx_buff_offset = model.bufferViews[idx_buff_accessor.bufferView].byteOffset + idx_buff_accessor.byteOffset;
		//uint32_t idx_buff_size = model.bufferViews[idx_buff_accessor.bufferView].byteLength; 
		uint32_t idx_buff_size = idx_buff_accessor.count * mesh_data.index_size; //byteLength should be equal to-> idx_buff_accessor.count * mesh_info.index_size
		mesh_data.index_data.resize(mesh_data.index_count);//resize to number of indeces
		memcpy(mesh_data.index_data.data(), &model.buffers[0].data[idx_buff_offset], idx_buff_size);
		...
		//Parse material
		std::vector<std::shared_ptr<ImageBase>> material_textures;
		parseGLTFMaterial(model.materials[primitive.material], model, material_textures, folder_directory);
	...
	MeshComponent AssetLoader::loadMeshComponent( const char* gltf_filepath)
	{
		...
		std::string filename = gltf_filepath;
		...
		for (auto& node : model.nodes)
		{
			...
			{ 
				processGLTFMeshPrimitive(primitive, model, sub_meshes, directory);
			}
		}
