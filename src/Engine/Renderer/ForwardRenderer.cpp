...
void ForwardRenderer::renderSceneECS(CommandBuffer& cmd_buffer)
{
	...
	Camera* main_cam;
	{
		auto& scene_view = m_scene->ecsRegistry()->view<CameraComponent>();
	...
	{
		auto& scene_view = m_scene->ecsRegistry()->view<LightProbeComponent>();
		for (ecs::EntityId entity : scene_view)
		{
	...
	//Fill point light buffer
	{
		...
		auto& scene_view = m_scene->ecsRegistry()->view<PointLightComponent>();
		for (ecs::EntityId entity : scene_view)
		{
	...
	//Fill directional light buffer
	{
		...
		uint32_t light_count = 0;
		auto& scene_view = m_scene->ecsRegistry()->view<DirectionalLightComponent>();
	...
	//Render each entity
	auto& scene_view = m_scene->ecsRegistry()->view<MeshComponent>();
	for (ecs::EntityId entity : scene_view)
	{
		...
		/* MOVE TO SCENE UPDATE SECTION. THIS IS LOGIC NOT RENDERING
		glm::mat4 rot = glm::rotate(glm::mat4(1.0f), glm::radians(frame_counter * 0.01f), glm::vec3(0., 1, 0));
		glm::vec3 trans_vec = glm::vec3{ model[3][0], model[3][1], model[3][2] };
		glm::mat4 trans_origin = glm::translate(glm::mat4(1.0f), -trans_vec);
		glm::mat4 trans_back = glm::translate(glm::mat4(1.0f), trans_vec);
	...
	//Render skybox
	{
		auto& scene_view = m_scene->ecsRegistry()->view<SkyboxComponent>();
		for (ecs::EntityId entity : scene_view)
		{
